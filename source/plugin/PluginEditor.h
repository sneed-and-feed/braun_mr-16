...
class BRAUN_MR16AudioProcessorEditor : public juce::AudioProcessorEditor,
                                       public juce::AudioProcessorValueTreeState::Listener,
                                       private juce::Timer
{
public:
    explicit BRAUN_MR16AudioProcessorEditor(BRAUN_MR16AudioProcessor&);
    ~BRAUN_MR16AudioProcessorEditor() override;
...
private:
    ...
    BRAUN_MR16AudioProcessor& processorRef;
    mr16::BraunLookAndFeel braunLookAndFeel;
    bool useNativeUI { false };

#if JUCE_WEB_BROWSER
    static juce::WebBrowserComponent::Options createWebOptions(BRAUN_MR16AudioProcessorEditor& editor);
    std::unique_ptr<juce::WebBrowserComponent> webComponent;
    bool initialSyncDone { false };

    // Coalescing array for all 32 APVTS parameters (prevents message loop stalls)
    static constexpr size_t kNumParams = 32;
